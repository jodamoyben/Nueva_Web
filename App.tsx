            ...
            {kpiData.map((kpi, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`text-xs font-black text-${kpi.color}-600 bg-${kpi.color}-50 px-2 py-1 rounded-full`}>
                    {kpi.change}
                  </span>
                </div>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              { label: lang === 'es' ? 'Banca y Finanzas' : 'Banking & Finance', icon: Shield, color: 'blue' },
              { label: lang === 'es' ? 'Sector Seguros' : 'Insurance Sector', icon: Zap, color: 'emerald' },
              { label: 'Contact Centers', icon: Globe, color: 'purple' },
              { label: 'Retail & E-comm', icon: Brain, color: 'indigo' }
            ].map((sector, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 30 }}
              ...
              >
                <motion.div
                  whileHover={{ rotate: 8 }}
                  className={`w-20 h-20 md:w-24 md:h-24 bg-${sector.color}-50 text-${sector.color}-600 rounded-[30px] flex items-center justify-center mx-auto mb-6 shadow-sm group-hover:bg-${sector.color}-600 group-hover:text-white transition-all duration-500`}
                >
